...
public final class ClientPacketHandler {
    ...
    public static void register() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(ModPackets.OPEN_OWNER_ID,
            (payload, ctx) -> {
                RegistryWrapper.WrapperLookup reg = ctx.client().world != null ? ctx.client().world.getRegistryManager() : null;
        ...
        ClientPlayNetworking.registerGlobalReceiver(ModPackets.OPEN_BUYER_ID,
            (payload, ctx) -> {
                ...
                ShopData data = ShopData.fromNbt(payload.data(), reg);
                ctx.client().execute(() -> {
                    if (ctx.client().currentScreen instanceof ShopBuyerScreen s) {
                        s.refreshData(data);
                    } else {
        ...
        ClientPlayNetworking.registerGlobalReceiver(ModPackets.SYNC_SHOP_ID,
            (payload, ctx) -> {
                ...
                ShopData data = ShopData.fromNbt(payload.data(), reg);
                ctx.client().execute(() -> {
                    if (ctx.client().currentScreen instanceof ShopOwnerScreen s) {
                        s.refreshData(data);
                    }
                });
            });
        ...
        ClientPlayNetworking.registerGlobalReceiver(ModPackets.OPEN_STORAGE_ID,
            (payload, ctx) -> {
                ...
                ShopData data = ShopData.fromNbt(payload.data(), reg);
                ctx.client().execute(() -> {
                    if (ctx.client().currentScreen instanceof StorageScreen s) {
                        s.refreshData(data);
                    } else {
        ...
        ClientPlayNetworking.registerGlobalReceiver(ModPackets.SHOP_NAV_ID,
            (payload, ctx) -> {
                ...
                ctx.client().execute(() -> {
                    if (ctx.client().currentScreen instanceof ShopBuyerScreen s) {
                        s.rebuild();
                    } else if (ctx.client().currentScreen instanceof ShopOwnerScreen s) {
                        s.rebuild();
                    }
                });
            });
    ...
    public static final class ShopNavigationHolder {
        ...
        public static String getPrevShop(String currentShop) {
            // Loop navigation: if at first shop, go to last shop
        ...
        public static String getNextShop(String currentShop) {
            // Loop navigation: if at last shop, go to first shop
            if (shopOwnerNames.size() <= 1) return null;
            int idx = getCurrentIndex(currentShop);
