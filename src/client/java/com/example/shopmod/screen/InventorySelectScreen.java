...
public class InventorySelectScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
        ...
        // === Tooltip: يظهر لما الماوس يقف فوق أي Item ===
        if (specialHov && !selectedStack.isEmpty()) {
            ctx.setTooltipForNextFrame(font, selectedStack, mx, my);
        } else {
            int hoveredSlot = getSlotAtPosition(mx, my);
            if (hoveredSlot >= 0 && !localInv[hoveredSlot].isEmpty()) {
                ctx.setTooltipForNextFrame(font, localInv[hoveredSlot], mx, my);
            }
        }
    }
    ...
    private void handleInventorySlotClick(int slot, int button) {
        ...
        // MC 26.3: button 1 = left click (full stack), button 0 = right click (half/split)
        if (button == 1) {
            if (!cursorStack.isEmpty()) {
            ...
            if (!cursorStack.isEmpty()) {
                ...
                int half = (stackInSlot.getCount() + 1) / 2; cursorStack = stackInSlot.copy(); cursorStack.setCount(half); stackInSlot.shrink(half); if (stackInSlot.getCount() <= 0) localInv[slot] = ItemStack.EMPTY;
            }
        }
    }
    ...
    private void handleSpecialSlotClick(int button) {
        ...
        if (button == 1) {
            ...
            if (!cursorStack.isEmpty()) {
                ...
                if (half > 0) { cursorStack = selectedStack.copy(); cursorStack.setCount(half); selectedStack.shrink(half); if (selectedStack.getCount() <= 0) selectedStack = ItemStack.EMPTY; }
            }
        }
    }
