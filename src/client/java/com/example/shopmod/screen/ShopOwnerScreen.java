...
public class ShopOwnerScreen extends Screen {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (mx >= sellSlotX && mx < sellSlotX + SLOT_SIZE && my >= sellSlotY && my < sellSlotY + SLOT_SIZE) {
            // Don't return pendingSell to inventory - it was never removed (InventorySelectScreen uses local copies)
            pendingSell = ItemStack.EMPTY; ClientPacketHandler.PendingSellHolder.clear();
            if (this.minecraft != null) this.minecraft.gui.setScreen(new InventorySelectScreen(shopName, shopData));
            return true;
        }
