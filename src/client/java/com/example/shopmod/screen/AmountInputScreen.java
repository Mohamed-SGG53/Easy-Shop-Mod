...
public class AmountInputScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
        ...
        ctx.centeredText(font, Component.literal(I18n.get("amount.title") + " - " + shopName), px+W/2, py+6, 0xFFFFFFFF);
        ctx.fill(px+W/2-12,py+30,px+W/2+12,py+54,0xFF444444);
        drawBorder(ctx,px+W/2-12,py+30,24,24,0xFFAAAAAA);
        ctx.item(displayStack, px+W/2-8, py+34);
        ctx.centeredText(font, displayStack.getHoverName(), px+W/2, py+60, 0xFFFFFFFF);
